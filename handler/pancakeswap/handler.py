...
from config import PancakeswapConfig

class PancakeswapHandler:
    def __init__(self):
        self.pancakeswap_config = PancakeswapConfig()
        self.web3 = Web3(Web3.HTTPProvider(self.pancakeswap_config.rpc_url))

    def findPool(self, token1Address, token2Address):
        contract = self.web3.eth.contract(
            address=Web3.to_checksum_address(self.pancakeswap_config.factory_contract),
            abi=self.pancakeswap_config.pancakeswap_factory_abi
        )

        for fee in self.pancakeswap_config.fee_tiers:
            try:
    ...
    def get_price(self, pool_address, token1_decimals, token2_decimals):
        pool = self.web3.eth.contract(
            ...
            abi=self.pancakeswap_config.pool_abi
        )
    ...
    def get_max_amount_of_pool(self, pool_address, token1_decimals, price_change_bps=50):
        pool = self.web3.eth.contract(address=Web3.to_checksum_address(pool_address), abi=self.pancakeswap_config.pool_abi)
